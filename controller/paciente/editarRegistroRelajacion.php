...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['editableData'])) {
    ...
    if (!empty($data)) {
        ...
        Relajacion_muscular::deleteById($bdd->link, $lastIdRegistro);
        ...
        foreach ($organizedData as $row) {
                ...
                $relajacion = new Relajacion_muscular(
                        $row['id_registro'],
                        $row['id_linea'],
                        $row['cell_1'],
                        $row['cell_2'],
        ...
        if($resultado) {
            ...
            $body = "<p>¡Su paciente ha modificado su registro para la práctica de la relajación muscular! Acceda a la app para revisarlo.</p>
